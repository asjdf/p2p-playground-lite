...
- **PSK 认证** - ✅ 已实现 (2026-01-19)
  - 使用预共享密钥 (PSK) 创建私有 P2P 网络
  - 32 字节（256 位）随机密钥，十六进制编码
  - 只有持有相同 PSK 的节点才能相互连接
  - 通过 `controller psk` 命令生成
- **连接白名单** - ✅ 已实现
  - 支持 `trusted_peers` 配置限制可连接的 peer ID
  - 提供 PSK + peer ID 双重保护
- **TLS 1.3 加密** - ✅ 默认启用
  - 所有 P2P 连接使用 TLS 1.3 或 Noise 协议加密
  - libp2p 自动协商，无需手动配置
  - 支持前向保密（Forward Secrecy）
- **证书认证** - 计划中（可作为 PSK 的替代或补充）
...
#### 3.3.3 传输加密
- **TLS 1.3** - ✅ 已实现，所有 P2P 连接默认加密
  - libp2p 自动协商 TLS 1.3（优先）或 Noise 协议
  - 基于 peer ID 的身份绑定，防止中间人攻击
  - 支持前向保密（Forward Secrecy）
- **PSK 网络隔离** - 与 TLS 配合使用
  - PSK 控制网络访问（谁能加入）
  - TLS 保护数据传输（数据保密性）
...
security:
  # PSK 网络认证（可选，用于创建私有 P2P 网络）
  enable_auth: true
  psk: "your-pre-shared-key-generated-by-controller-psk-command"  # 64 个十六进制字符
...
security:
  # PSK 网络认证（必须与 Controller 相同）
  enable_auth: true
  psk: "your-pre-shared-key-generated-by-controller-psk-command"  # 与 Controller 相同

  # 可信节点白名单（可选，提供额外保护）
  trusted_peers:
    - "12D3KooWBaiJyfJwTo4HCtGv3qBkZnMcuK3KvHmvW7Example1"
    - "12D3KooWBaiJyfJwTo4HCtGv3qBkZnMcuK3KvHmvW7Example2"

  # 应用包签名验证
  allow_unsigned_packages: false  # 默认 false，拒绝未签名包
  # 可信公钥目录
...
**配置说明**：
- **PSK 认证**：`enable_auth: true` 和相同的 `psk` 值用于创建私有网络
- **白名单**：`trusted_peers` 提供额外的 peer ID 级别保护（可选）
- **签名验证**：`allow_unsigned_packages: false` 默认拒绝未签名的应用包
- **公钥目录**：将 Controller 的公钥文件复制到 `~/.p2p-playground/keys/trusted/*.pub`

## 7. 开发路线图

### Phase 1: 核心功能（MVP）
- [x] **P2P 网络基础** - ✅ libp2p 集成已完成
  - libp2p host 创建和配置
  - TCP + QUIC 传输协议支持
  - TLS 1.3 + Noise 安全传输
- [x] **节点发现** - ✅ mDNS 已实现
...
- [x] **基础进程管理** - ✅ 已完成
  ...
  - 日志捕获（stdout/stderr）
- [x] **CLI 工具** - ✅ 轻量版已完成
  - controller: deploy, list, logs, nodes, keygen, sign, psk
  - daemon: start, status

### Phase 2: 安全和稳定性
- [x] **节点身份认证** - ✅ PSK 认证已实现 (2026-01-19)
- [x] **TLS 1.3 传输加密** - ✅ 默认启用 (2026-01-19)
- [x] **代码签名和验证** - ✅ Ed25519 签名已实现，默认强制要求签名
- [x] **健康检查** - ✅ 进程/HTTP/TCP 检查已实现并集成到 runtime
- [x] **自动重启机制** - ✅ 已实现 StartWithAutoRestart
- [ ] 资源限制（cgroups）
- [x] 日志收集 - ✅ 基本功能已完成
...
1. **网络安全**
   - ✅ **PSK 认证已实现** - 使用预共享密钥限制节点加入
   - ✅ **TLS 1.3 加密** - 所有 P2P 连接默认加密
   - **定期轮换 PSK** - 建议每季度或每半年轮换一次
   - **使用白名单** - 生产环境建议配置 `trusted_peers`
   - **监控异常连接** - 关注连接拒绝日志

2. **代码安全**
   - ✅ **强制签名验证已实现** - 默认拒绝未签名包
   - **定期审计公钥** - 检查 `keys/trusted/` 目录中的公钥列表
   - **实施最小权限** - 每个环境使用不同的 PSK 和签名密钥
   - **密钥分离** - 开发/测试/生产环境使用独立的密钥对
...
4. **数据安全**
   - **密钥存储** - 使用 0600 权限保护私钥文件
   - **PSK 保护** - PSK 应如密码一样妥善保管
   - 敏感配置加密存储（计划中）
   - 日志脱敏（计划中）
   - 安全删除旧版本应用
